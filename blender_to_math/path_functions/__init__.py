...
def function_1d_to_bmesh(f, **kwargs):
    ...
    if t_num is None or t_num < 2:
        raise ValueError('Need t_num')
    t1 = kwargs.get('t1')
    if t1 is None:
        raise ValueError('Need t1')
    t2 = kwargs.get('t2')
    if t2 is None:
    ...
    for i in range(t_num):
        ...
        t += t_inc
        if i > 0:
            bm.edges.new((vert_last, vert))
    if cyclic:
        bm.edges.new((vert, vert_first))
    return bm


def function_2d_to_bmesh(f, **kwargs):
    u_num = kwargs.get('u_num')
    if u_num is None or u_num < 2:
        raise ValueError('Need u_num')
    u1 = kwargs.get('u1')
    if u1 is None:
        raise ValueError('Need u1')
    u2 = kwargs.get('u2')
    if u2 is None:
        raise ValueError('Need u2')
    u_cyclic = kwargs.get('u_cyclic') or False

    v_num = kwargs.get('v_num')
    if v_num is None or v_num < 2:
        raise ValueError('Need v_num')
    v1 = kwargs.get('v1')
    if v1 is None:
        raise ValueError('Need v1')
    v2 = kwargs.get('v2')
    if v2 is None:
        raise ValueError('Need v2')
